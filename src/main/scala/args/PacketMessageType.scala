...
class PacketMessageType(msgType: MessageType.Value) extends ForgeArgument {
  ...
  override def toArgument() : String = msgType match {
      ...
      case MessageType.GsnResolution => "--msg-type gsn_resolution"
        
      case MessageType.GtnGeosubNodes => "--msg-type gtn_geosub_nodes"
      case MessageType.GtnRegistration => "--msg-type gtn_registration"
